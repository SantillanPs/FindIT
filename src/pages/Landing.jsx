...
const Landing = () => {
  ...
  // Hardcoded Configuration (Replaced dynamic site_configs table)
  const SHOW_ANNOUNCEMENT = false;
  const ANNOUNCEMENT_TEXT = "";
  const SHOW_LEADERBOARD = true;
  const SHOW_IDENTIFIED = true;
  ...
  return (
    <div className="landing-page space-y-10 md:space-y-24 pb-20 relative overflow-hidden min-h-screen text-white">
        ...
        <div className="max-w-7xl mx-auto text-center px-6 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 md:gap-10 max-w-2xl mx-auto">
            <Button 
                ...
                className="lt-cta duration-0 h-14 md:h-16 px-8 md:px-14 rounded-2xl font-bold text-xs md:text-sm uppercase tracking-[0.2em] bg-white hover:bg-slate-200 text-black shadow-2xl shadow-sky-500/10 group"
            >
                Report Missing
            ...
            <Button 
                ...
                className="lt-outline duration-0 h-14 md:h-16 px-8 md:px-14 rounded-2xl font-bold text-xs md:text-sm uppercase tracking-[0.2em] border-white/10 bg-white/5 backdrop-blur-md hover:bg-white/10 text-white group"
            >
                Found Something
      ...
      {SHOW_IDENTIFIED !== false && items.some(i => i.identified_name || i.identified_student_id) && !itemsLoading && (
        <section className="max-w-7xl mx-auto px-6 md:px-12 py-12 md:py-24">
          <div className="flex items-center gap-6 mb-16 px-4 md:px-0">
            <div className="lt-badge w-16 h-16 rounded-2xl bg-sky-500/10 border border-white/5 flex items-center justify-center text-sky-400">
          ...
          <div className="flex overflow-x-auto gap-8 pb-10 no-scrollbar snap-x px-6 md:px-0 mask-horizontal-fade">
            {items.filter(i => i.identified_name || i.identified_student_id).map(item => (
              <Card 
                key={item.id}
                className="lt-surface min-w-[320px] md:min-w-[460px] snap-start border border-white/10 bg-slate-900/40 backdrop-blur-xl overflow-hidden flex flex-col relative group transition-all duration-500"
              >
                <div className="absolute top-6 left-6 z-10">
                   <div className="lt-overlay inline-flex items-center gap-2 py-1.5 px-4 rounded-full border border-white/10 bg-black/60 backdrop-blur-md">
                      <IdCard className="h-3 w-3 text-sky-400" />
                      <span className="lt text-[10px] font-black text-white uppercase tracking-widest italic">Identity Linked</span>
                   </div>
                </div>
                
                 <div className="lt-photo aspect-[16/9] md:h-60 bg-slate-950/50 flex items-center justify-center relative overflow-hidden">
                    {item.photo_url ? (
                      <img 
                 ...
                 <CardContent className="p-8 text-left space-y-6 flex-grow">
                    <div className="space-y-2">
                       ...
                       <h3 className="lt text-2xl md:text-3xl font-black text-white italic tracking-tight uppercase line-clamp-1">
                    ...
                    <p className="text-slate-400 text-xs md:text-sm leading-relaxed line-clamp-2 font-medium italic">
                      A <span className="lt text-white font-bold">{item.title}</span> recovered at <span className="text-sky-400 font-bold">{item.location}</span>.
                    </p>

                   <div className="lt-divider pt-8 border-t border-white/5 flex gap-4">
                      <Button 
                      ...
                      <Button 
                         ...
                         className="lt-outline w-14 h-14 p-0 rounded-xl border-white/10 bg-white/5 hover:bg-white/10"
                      >
      ...
      {SHOW_LEADERBOARD !== false && (
        <section className="lt-divider relative py-12 md:py-16 overflow-hidden border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-16 md:gap-24">
            <div className="lg:w-1/2 space-y-10 text-left">
              <div className="lt-badge inline-flex items-center gap-2 py-1.5 px-4 rounded-full border border-sky-500/30 bg-sky-500/5 backdrop-blur-md">
              ...
              <h2 className="text-5xl md:text-8xl font-extrabold text-white tracking-tighter leading-[0.85] uppercase">
                 Community <br/><span className="lt-gradient bg-gradient-to-br from-white via-white/80 to-slate-500 bg-clip-text text-transparent">Honor Roll</span>
              </h2>
              ...
              <div className="lt-tabs flex flex-wrap gap-2 p-1.5 bg-white/5 border border-white/10 rounded-2xl w-fit backdrop-blur-md">
                  <Button 
                      variant={leaderboardType === 'students' ? 'default' : 'ghost'}
                      onClick={() => setLeaderboardType('students')}
                      className={`rounded-xl px-5 font-bold text-[10px] uppercase tracking-widest h-11 ${leaderboardType === 'students' ? 'bg-white text-black shadow-xl' : 'lt-muted text-slate-500 hover:text-white'}`}
                  >
                  ...
                  <Button 
                      ...
                      className={`rounded-xl px-5 font-bold text-[10px] uppercase tracking-widest h-11 ${leaderboardType === 'colleges' ? 'bg-white text-black shadow-xl' : 'lt-muted text-slate-500 hover:text-white'}`}
                  >
            ...
            <div className="lg:w-1/2 w-full relative">
              <Card className="lt-surface p-2 md:p-6 border border-white/10 bg-slate-900/40 backdrop-blur-xl shadow-2xl">
                  <div className="space-y-2">
                      {!masterLoading && leaderboardType === 'students' ? (
                          LEADERBOARD_FROM_MASTER?.students?.slice(0, showFullLeaderboard ? 10 : 3).map((student, i) => (
                              <div key={i} className="lt-row flex items-center justify-between p-3.5 md:p-4 rounded-xl border border-transparent hover:border-white/5 hover:bg-white/5 group">
                                  <div className="flex items-center gap-4 md:gap-5 flex-1 min-w-0">
                                      ...
                                      <div className="text-left min-w-0 flex-1">
                                          <p className="lt text-[12px] md:text-[13px] font-bold text-white tracking-wider uppercase group-hover:text-sky-400 truncate">
                                  ...
                                  <div className="flex items-center gap-4 md:gap-6 shrink-0 ml-4">
                                      <p className="lt text-[11px] md:text-[12px] font-bold text-white tracking-[0.2em] uppercase">{student.integrity_points} <span className="text-slate-500 font-medium">IP</span></p>
                          ...
                          LEADERBOARD_FROM_MASTER?.departments?.slice(0, showFullLeaderboard ? 10 : 3).map((col, i) => (
                              <div key={i} className="lt-row flex items-center justify-between p-3.5 md:p-4 rounded-xl border border-transparent hover:border-white/5 hover:bg-white/5 group">
                                  <div className="flex items-center gap-4 md:gap-5 flex-1 min-w-0">
                                      ...
                                      <div className="text-left min-w-0 flex-1">
                                          <p className="lt text-[12px] md:text-[13px] font-bold text-white tracking-wider uppercase group-hover:text-sky-400 line-clamp-2 leading-tight">
                                              {col.department}
                                          </p>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-4 md:gap-6 shrink-0 ml-4">
                                      ...
                                      <Building2 className="h-5 w-5 text-slate-700 group-hover:text-slate-500" />
                                  </div>
                              </div>
                          ))
                      )}
                  ...
                  {(leaderboardType === 'students' ? LEADERBOARD_FROM_MASTER?.students : LEADERBOARD_FROM_MASTER?.departments)?.length > 3 && (
                      <div className="lt-divider border-t border-white/5">
                          <Button 
                              variant="ghost" 
                              onClick={() => setShowFullLeaderboard(!showFullLeaderboard)}
                              className="w-full h-12 lt-muted text-slate-500 hover:text-white font-bold text-[10px] uppercase tracking-[0.2em] group"
                          >
                              {showFullLeaderboard ? (
                                  <>
      ...
      <section id="browse" className="lt-divider py-12 md:py-20 border-t border-white/5 relative">
        <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-12 md:space-y-24">
          <div className="lt-divider flex flex-col md:flex-row justify-between items-start md:items-end gap-12 md:gap-24 border-b border-white/5 pb-16 md:pb-24">
             <div className="text-left space-y-6">
                <div className="flex items-center gap-6">
                  ...
                  <div className="lt-badge py-2 px-6 rounded-full border border-sky-500/30 bg-sky-500/5 backdrop-blur-md hidden md:flex items-center gap-3">
                  ...
                  {!isSearchExpanded && searchQuery === '' ? (
                    ...
                    >
                      <Button
                        ...
                        className="lt-surface h-12 md:h-14 px-6 md:px-8 rounded-2xl border border-white/10 bg-slate-900/40 backdrop-blur-xl text-slate-400 font-bold text-[11px] uppercase tracking-widest hover:bg-white/5 shadow-2xl flex items-center gap-4 group"
                      >
                        <Search className="h-4 w-4 text-slate-500 group-hover:text-sky-400" />
                    ...
                    >
                      <Search className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-500 group-focus-within:text-sky-400 z-20" />
                      <Input 
                        ref={searchInputRef}
                        type="text" 
                        placeholder="Search inventory..." 
                        className="lt-surface h-12 md:h-14 pl-12 pr-10 rounded-2xl border-white/10 bg-slate-900/40 backdrop-blur-xl text-sm font-bold uppercase tracking-tight focus-visible:ring-sky-500/30 placeholder:text-slate-600 shadow-2xl w-full"
                        value={searchQuery}
                      ...
                      <button 
                        ...
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-full z-20"
                      >
          ...
          <div className="flex items-center gap-4 md:gap-6 overflow-x-auto no-scrollbar pb-6 mask-horizontal-fade">
             <Button 
               ...
               className={`h-11 md:h-14 px-8 md:px-12 rounded-2xl text-[10px] md:text-xs font-black uppercase tracking-[0.2em] italic shrink-0 border ${
             ...
             {(CATEGORIES_FROM_MASTER || []).map(cat => (
                <Button 
                  ...
                  className={`h-11 md:h-14 px-8 md:px-12 rounded-2xl text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] shrink-0 border ${
          ...
          {itemsLoading && items.length === 0 ? (
              ...
              <Card className="lt-surface py-24 md:py-40 text-center border border-white/10 bg-slate-900/40 backdrop-blur-xl rounded-[3rem]">
      ...
      <section id="lost-reports" className="lt-divider py-12 md:py-20 border-t border-white/5 relative">
        <div className="max-w-7xl mx-auto px-6 md:px-12 space-y-12 md:space-y-24">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12 md:gap-24">
            <div className="text-left space-y-6">
              <div className="flex items-center gap-6">
                ...
                <div className="lt-badge-rose py-2 px-6 rounded-full border border-rose-500/30 bg-rose-500/5 backdrop-blur-md hidden md:flex items-center gap-3">
          ...
          {reportsLoading && lostReports.length === 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
              {[1,2,3,4].map(i => <div key={i} className="lt-skeleton h-64 bg-white/5 animate-pulse rounded-2xl border border-white/5"></div>)}
            </div>
