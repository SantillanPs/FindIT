    ...
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="flex items-center gap-4 border-b border-white/5 pb-6 overflow-x-auto scrollbar-hide">
         ...
         <div className="ml-auto pl-4 border-l border-white/5">
            <button 
              onClick={selectAll}
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all bg-white/5 text-slate-400 hover:text-white hover:bg-white/10"
            >
              <i className={`fa-solid ${selectedIds.length === filteredItems.length && filteredItems.length > 0 ? 'fa-check-square' : 'fa-square'} text-[10px]`}></i>
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] w-full max-w-2xl px-4 pointer-events-none">
          <div className="glass-panel bg-slate-900/90 border-t-2 border-uni-500/30 p-4 rounded-[2rem] shadow-2xl shadow-black/50 pointer-events-auto flex items-center justify-between gap-6 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-3 pr-2">
               {pendingIntakeIds.length > 0 && (
                 <button 
                  ...
                  className="bg-uni-600 hover:bg-uni-500 text-white px-6 py-3 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center gap-3 shadow-lg shadow-uni-600/20"
                 >
