...
const InventoryCard = ({ item, matches, pendingClaims, navigate, setSearchTerm, handleStatusUpdate, setShowReleaseModal, setReleaseForm, actionLoading, isSelected, onToggleSelect }) => {
  ...
  return (
    <div key={item.id} className={`glass-panel border rounded-2xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:bg-white/[0.05] transition-all group overflow-hidden relative ${
       isSelected ? 'border-uni-500/50 bg-uni-500/5' :
       ...
       <div className={`flex items-center gap-5 pl-10 transition-all ${isSelected ? 'translate-x-0' : 'md:-translate-x-2 group-hover:translate-x-0'}`}>
          {/* Visual ID */}
          <div className="w-12 h-12 bg-slate-900 rounded-xl border border-white/10 flex items-center justify-center text-xl relative flex-shrink-0">
             {CATEGORIES.find(c => c.id === item.category)?.emoji || '📦'}
             <div className="absolute -bottom-1 -right-1 bg-slate-800 text-[8px] font-black text-slate-300 px-1 py-0.5 rounded border border-white/10">
                #{item.id.toString().padStart(4, '0')}
             </div>
          </div>

          {/* Primary Info: Location as Title (User feedback) */}
          <div className="space-y-1 text-left">
             <h4 className="text-[13px] font-black text-white uppercase tracking-widest leading-tight group-hover:text-uni-400 transition-colors">
                {item.location_zone}
             </h4>
             <div className="flex items-center gap-2">
                <span className="text-[9px] font-black text-uni-400 uppercase tracking-widest bg-uni-400/10 px-2 py-0.5 rounded border border-uni-400/20">
                    {item.item_name}
                </span>
       ...
       <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 flex-grow max-w-xl">
          <div className="space-y-1 text-left">
             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Registry Ownership</p>
             <p className="text-[11px] font-black text-white uppercase tracking-wide">
                {item.finder_id ? (
                    <Link to={`/admin/profile/${item.finder_id}`} className="hover:underline decoration-slate-400">
          ...
          <div className="space-y-2 border-l border-white/5 pl-4 hidden sm:block text-left">
             <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Insight Analysis</p>
             <div className="flex flex-wrap gap-2">
                {itemMatches.length > 0 ? (
                    <div className="flex items-center gap-1.5 bg-uni-500/10 px-2 py-0.5 rounded border border-uni-500/20">
                       <span className="text-[9px] font-black text-uni-400">{itemMatches.length} matches</span>
                       <div className="w-1.5 h-1.5 rounded-full bg-uni-400"></div>
                    </div>
                ) : (
                    <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tighter">Searching...</span>
                )}
                
                {itemClaims.length > 0 && (
                    <div className="flex items-center gap-1.5 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                       <span className="text-[9px] font-black text-amber-500">{itemClaims.length} Claims</span>
                    </div>
                )}
                ...
                {itemMatches.length > 0 && (
                    <button 
                        ...
                        className="text-[9px] font-black text-uni-400 uppercase tracking-widest hover:underline decoration-uni-500/50 underline-offset-4"
                    >
                        Inspect →
                    </button>
                )}
       ...
       <div className="w-full md:w-auto flex flex-row md:flex-col items-center md:items-end gap-3 border-t md:border-none border-white/5 pt-3 md:pt-0">
          <div className={`px-2 py-1 rounded-md text-[8px] font-black uppercase tracking-[0.1em] border ${
            ...
            <>
              {item.status === 'reported' ? (
                  <button 
                    ...
                    className="flex-grow md:w-36 bg-uni-600 text-white text-[10px] font-black uppercase tracking-widest py-2.5 rounded-xl hover:bg-uni-500 transition-all flex items-center justify-center gap-2 active:scale-95"
                  >
